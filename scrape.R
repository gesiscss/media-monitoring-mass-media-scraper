...
scraper_headlines <- function(folder, rss_feed, agency) {
  # load packages
  require(stringr)
  require(magrittr)
  require(xml2)
  require(rvest)
  rss_out_list <- lapply(rss_feed, read_xml)
  # write raw RSS
  dir.create(folder, showWarnings = FALSE, recursive = TRUE)
  datetime <- format(as.POSIXct(Sys.time(), tz = Sys.timezone()), usetz = TRUE)  %>% as.character() %>% str_replace_all("[ :]", "-")
  rss_feed <- rss_feed %>% str_replace("@@RSS|\\/index.rss\\$|index.rss|\\/index|\\/rss", "")
  filepaths <-paste0(folder, "/", agency, "-", datetime, "-", basename(rss_feed), ".rss")
...
scraper_articles <- function(folderInput, folderOutput, agency) {
  ...
  if (length(xmls) > 1)
  { xmls <- xmls[(length(xmls)-(length(rss_feed)-1)):length(xmls)] }# pick only newest files}
...
for (agentur in agency)
{
  ...
  rss_feed <- sapply(Filter(function(x) x$agency == agentur, rss_feeds), "[[", "url")
  scraper_headlines(folder = input, rss_feed, agency = agentur)
